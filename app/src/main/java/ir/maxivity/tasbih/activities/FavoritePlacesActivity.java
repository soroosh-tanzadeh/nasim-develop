...
public class FavoritePlacesActivity extends BaseActivity {
    ...
    private void getFavoritePlaces() throws NullPointerException {
        dialog.show();
        RequestBody pass = RequestBody.create(MediaType.parse("text/plain"), application.getToken());
    ...
    private void getPlacesById(String id) throws NullPointerException {
        ...
        body.id = id;
        places.clear();
        RequestBody userId = RequestBody.create(Utilities.TEXT, application.getUserId());
        application.api.getPlace(RequestBody.create(Utilities.JSON, Utilities.createBody(body)), userId).
